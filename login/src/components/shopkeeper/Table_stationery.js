...
function Table_stationery() {
  ...
  useEffect(() => {
   ...
   axios.get("http://localhost:9002/api/all-product_stationerys")
  ...
  function handleDelete(id) {
    console.log(id)
   axios.delete(`http://localhost:9002/api/product_stationerys/${id}`)
  }
}
function EditItem({ current, datas, setData }) {
  ...
  function handleUpdate(id,name,quantity,price) {
    console.log("update",id)
    axios.put(`http://localhost:9002/api/product_stationerys/${id}`,{
      name,
      quantity,
      price,
    }
...
function AddItem({ setData }) {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post("http://localhost:9002/api/product_stationerys", product)
      .then((res) => {
